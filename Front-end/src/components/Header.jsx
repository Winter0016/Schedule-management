...
export const Header = ()=>{
    ...
    const sendAudioToServer = async (audioBlob) => {
        ...
        try {
            const response = await fetch('https://34.201.151.117:3000//voice-receive', {
                method: 'POST',
                body: formData,
            });
    ...
    const [turnonmic, setturnonmic] = useState(false);
        ...
        const addactivity2 = async (name,description,day,bgcolor,textcolor,timestart,timeend,modifyacname) => {
            ...
            try {
              ...
              const response = await fetch("https://34.201.151.117:3000//add-daily", {
                method: "POST",
    ...
    const handleSendMessage = async (text) => {
        if (text) {
            ...
            try {
                setaithinking(true);
                const response = await axios.post('https://34.201.151.117:3000//ask', {
                    username: loggedusername,
                    planname: selectedOption,
                    question: text,
                });
