...
export const Chat = () => {
    ...
    const handleSendMessage = async (text) => {
        if (text) {
            ...
            try {
                setaithinking(true);
                const response = await axios.post('http://34.201.151.117:3000/ask', {
    ...
    const sendAudioToServer = async (audioBlob) => {
        ...
        try {
            const response = await fetch('http://34.201.151.117:3000/voice-receive', {
                method: 'POST',
                body: formData,
            });
    ...
    const addactivity2 = async (name,description,day,bgcolor,textcolor,timestart,timeend,modifyacname) => {
        ...
        try {
          ...
          const response = await fetch("http://34.201.151.117:3000/add-daily", {
            method: "POST",
