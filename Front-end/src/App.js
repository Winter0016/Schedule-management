...
function App() {
  ...
  const updateTodayTask = async (plan) => {
    ...
    try {
      ...
      if (plan && selectedOption) {
        ...
        const response = await axios.post('http://100.27.190.222:3000/update-todaytask', {
  ...
  const getplan = async (req, res) => {
    try {
      const response = await fetch("http://100.27.190.222:3000/plan", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
async function subscribe(username,title,body,time) {
  ...
  await axios.post('http://100.27.190.222:3000/subscribe', subscription,username,title,body,time);
