...
function App() {
  ...
  const updateTodayTask = async (plan) => {
    ...
    try {
      ...
      if (plan && selectedOption) {
        ...
        const response = await axios.post('https://34.201.151.117/update-todaytask', {
  ...
  const getplan = async (req, res) => {
    try {
      const response = await fetch("https://34.201.151.117/plan", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
async function subscribe(username,title,body,time) {
  ...
  await axios.post('https://34.201.151.117/subscribe', subscription,username,title,body,time);
